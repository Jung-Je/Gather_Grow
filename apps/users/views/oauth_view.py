        ...
        if isinstance(data, dict) and "code" in data and "access_token" not in data:
            ...
            try:
                ...
                if token_response.status_code != 200:
                    ...
                    logger.error(
                        f"카카오 토큰 교환 실패: {token_response.status_code} - {error_detail}"
                    )
                    return APIResponse.bad_request(
                        message="카카오 인증 코드를 토큰으로 교환하는데 실패했습니다.",
                        data={
                            "error": f"{token_response.status_code} - {error_detail}",
                            "redirect_uri_used": settings.SOCIAL_AUTH_CONFIG["KAKAO"][
                                "REDIRECT_URI"
                            ],
                        },
                    )
        ...
        try:
            ...
            if isinstance(data, dict) and "code" in data and "access_token" not in data:
                ...
                token_data = {
                    ...
                    "client_secret": settings.SOCIAL_AUTH_CONFIG["GOOGLE"][
                        "SECRET_KEY"
                    ],
                    "redirect_uri": settings.SOCIAL_AUTH_CONFIG["GOOGLE"][
                        "REDIRECT_URI"
                    ],
                    "grant_type": "authorization_code",
                }
                ...
                try:
                    ...
                    if token_response.status_code != 200:
                        error_detail = (
                            token_response.json() if token_response.text else {}
                        )
                        logger.error(
                            f"구글 토큰 교환 실패: {token_response.status_code} - {error_detail}"
                        )
                        return APIResponse.bad_request(
                            message="구글 인증 코드를 토큰으로 교환하는데 실패했습니다.",
                            data={
                                "error": f"{token_response.status_code} - {error_detail}",
                                "redirect_uri_used": settings.SOCIAL_AUTH_CONFIG[
                                    "GOOGLE"
                                ]["REDIRECT_URI"],
                            },
                        )
                    ...
                    new_request.authenticators = getattr(
